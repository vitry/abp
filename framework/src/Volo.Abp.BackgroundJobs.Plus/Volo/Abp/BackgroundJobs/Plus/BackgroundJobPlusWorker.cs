...
public class BackgroundJobPlusWorker : AsyncPeriodicBackgroundWorkerBase, IBackgroundJobPlusWorker
{
    protected const string DistributedLockName = "AbpBackgroundJobPlusWorker";

    protected AbpBackgroundJobOptions JobOptions { get; }

    protected AbpBackgroundJobPlusWorkerOptions WorkerOptions { get; }

    protected IAbpDistributedLock DistributedLock { get; }

    public BackgroundJobPlusWorker(
        AbpAsyncTimer timer,
        IOptions<AbpBackgroundJobOptions> jobOptions,
        IOptions<AbpBackgroundJobPlusWorkerOptions> workerOptions,
    ...
    protected override async Task DoWorkAsync(PeriodicBackgroundWorkerContext workerContext)
    {
        await using (var handler = await DistributedLock.TryAcquireAsync(DistributedLockName, cancellationToken: StoppingToken))
        {
            if (handler != null)
            {
                ...
                var serializer = workerContext.ServiceProvider.GetRequiredService<IJsonSerializer>();
                ...
                foreach (var jobInfo in waitingJobs)
                {
                    ...
                    try
                    {
                        ...
                        var jobArgs = serializer.Deserialize(jobConfiguration.ArgsType, jobInfo.JobArgs);
